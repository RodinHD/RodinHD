...
class NeRFRenderer(nn.Module):
    def __init__(self,
        ...
        print("Render using grid size: ", self.grid_size)
    ...
    def run(self, triplane, rays_o, rays_d, num_steps=128, upsample_steps=128, bg_color=None, perturb=False, **kwargs):
        ...
        # query SDF and RGB
        density_outputs = self.density(triplane, xyzs.reshape(-1, 3))
        ...
        if upsample_steps > 0:
            ...
            new_density_outputs = self.density(triplane, new_xyzs.reshape(-1, 3))
        ...
        rgbs = self.color(triplane, xyzs.reshape(-1, 3), dirs.reshape(-1, 3), mask=mask.reshape(-1), **density_outputs)
    ...
    def render(self, triplane, rays_o, rays_d, staged=False, max_ray_batch=4096, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        # return: pred_rgb: [B, N, 3]
