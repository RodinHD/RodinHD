...
def get_named_beta_schedule(schedule_name, num_diffusion_timesteps):
    ...
    if schedule_name == "linear":
        ...
        betas = betas_for_alpha_bar(
            num_diffusion_timesteps,
            lambda t: cosine_schedule(t, start=0.2, end=1, tau=5),
        )
        return betas
...
class ModelMeanType(enum.Enum):
    ...
    V = enum.auto() # the model predicts v in distillation diffusion
...
class GaussianDiffusion:
    ...
    def p_sample(
        ...
        sample = out["mean"] + nonzero_mask * th.exp(0.5 * out["log_variance"]) * noise   #* t.view(t.shape[0],1,1,1) /100.
    ...
    def training_losses(self, model, x_start, t, vgg, adv, tvreg, model_kwargs=None, noise=None):
        """
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            ...
            if tvreg is not None:
                predicted_x0 = self._predict_xstart_from_eps(x_t=x_t, t=t, eps=model_output).clamp(-0.6, 0.6)
                terms["L1"] = th.mean(th.abs(predicted_x0)) * 0.001
                terms["TV"] = tvreg(predicted_x0) * 0.0005
            else:
                terms["L1"] = 0.
                terms["TV"] = 0.
            ...
            if "vb" in terms:
                terms["loss"] = terms["mse"] + terms["vb"] + terms["perceptual"] + terms["adv"] + terms["TV"] + terms["L1"]
            else:
    ...
    def training_forward(self, model, x_start, t, model_kwargs=None, noise=None, min_snr_k=-1):
        if model_kwargs is None:
            model_kwargs = {}
        if noise is None:
            noise = th.randn_like(x_start)
        ...
        if min_snr_k > 0:
            ...
            snr = (alpha / sigma) ** 2
            loss_weight = th.stack([snr, min_snr_k * th.ones_like(t)], dim=1).min(dim=1)[0]
 
        return model_output, loss_weight, target

    def _get_min_snr_loss_weight(self, t, min_snr_k=5):
        alpha = _extract_into_tensor(self.sqrt_alphas_cumprod, t, t.shape)
        sigma = _extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, t.shape)
        snr = (alpha / sigma) ** 2
        loss_weight = th.stack([snr, min_snr_k * th.ones_like(t)], dim=1).min(dim=1)[0]
